...
//        sites[bottom] = OPEN;
    ...
    public void open(int i, int j)          // open site (row i, column j) if it is not open already
    {
        ...
        int xy = xy(i, j);
//        StdOut.println(String.format("Opening - x(%d) y(%d) xy(%d)",i,j,xy));
        sites[xy] = true;//OPEN;
...
//        return  isOpen(i, j) && (
//                ( union.connected(xy(i, j), top) )                       ||
//                ( valid(i - 1, j) && union.connected(xy(i - 1, j), top)) ||
//                ( valid(i + 1, j) && union.connected(xy(i + 1, j), top)) ||
//                ( valid(i, j + 1) && union.connected(xy(i, j + 1), top)) ||
//                ( valid(i, j - 1) && union.connected(xy(i, j - 1), top))
//        );
