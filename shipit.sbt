...
def shipitParser(state: State): Parser[String] = {
...
def shipitAction(state: State, version: String): State = {
  "checkNoLocalChanges" ::
    "clean" ::
    ("all test it:test" ::
      s"git tag ${version}" ::
...
val shipitHelp = Help(
  Seq(
    "shipit" -> "Runs the shipit script for a given version number"
  ),
  Map(
    "shipit" ->
      """|Runs our shipit script.  This will:
        |1. Run all the tests.
...
val shipitCommand = Command("shipit", shipitHelp)(shipitParser)(shipitAction)
commands += shipitCommand