...
object IO2c {
  ...
  import scratchpad.ch7parallelism.Nonblocking._
  ...
  */
  ...
  def runTrampoline[A](a: Free[Function0, A]): A =
    a match {
      case Return(v) => v
      case Suspend(s) => s()
      case FlatMap(a1, f) => a1 match {
        case Return(v) => runTrampoline(f(v))
        case Suspend(s) => runTrampoline(f(s()))
        case FlatMap(a0, f0) => runTrampoline(a0.flatMap(a => f0(a) flatMap (f)))
      }
    }
