package scratchpad.ch15Streams

import scratchpad.ch15Streams.GeneralizedStreamTransducers.Process.Process1
import scratchpad.iomonad.{IO,Monad,Free,unsafePerformIO}
import scala.io
...
input (`Await`) or signaling termination via `Halt`.
  ...
  sealed trait Process[I,O] {
    ...
    def mapV[O2](f: O => O2): Process[I,O2] = this |> lift(f)
    /*
  ...
  object Process {
    ...
    import scratchpad.iomonad.Monad
    ...
    def mean: Process[Double,Double] = {
      def go(sm:Double,cnt:Double):Process[Double,Double] = {
        await( i => emit(
          (sm + i) / cnt,
          go(sm + i, cnt + 1)
        ))
    ...
    val mean2 = (sum zip count) |> lift({case(s,c) => s/c})
    /*
     * Exercise 6: Implement `zipWithIndex`.
     *
     * See definition on `Process` above.
     */
    def zipWithIndex[I] = (id[I] zip count) |> lift({case(i,c) => (i,c -1) })
...
//    def converter() = {
//      processFile(new java.io.File("asdsad"), convertFahrenheit, )
//    }
  }
}
    ...
    def runLog[O](src: Process[IO,O]): IO[IndexedSeq[O]] = IO {
      ...
      def go(cur: Process[IO,O], acc: IndexedSeq[O]): IndexedSeq[O] =
        cur match {
          ...
          case Await(req,recv) =>
            val next =
              try recv(Right(scratchpad.iomonad.unsafePerformIO(req)(E)))
...
`Process` to a `Sink`.
    ...
    import scratchpad.iomonad.IO0.fahrenheitToCelsius
