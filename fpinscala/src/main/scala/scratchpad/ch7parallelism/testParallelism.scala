...
//    }
  ...
  val zeroInt = 0
  val zeroList = Nil
  val intAdd:(Int,Int)=>Int = _+_
  val xsAdd:(List[Int],List[Int])=>List[Int] = _++_
  implicit val intAdder = (intAdd, zeroInt)
  implicit val listAdder = (xsAdd, zeroList)

  println( sum(Vector(1,2,3,4)) )
//  println( sum(Vector(List(1,2),List(3,4))))
}
